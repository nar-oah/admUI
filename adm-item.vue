<template>
  <view id="itemWrap" :class="[`item-wrap item-wrap-${theme}`]">
    <view class="border">
      <text
        id="itemBorder"
        :class="[`text text-${theme}`]"
        :style="{ 'margin-left': `${border.random[0]}rpx` }"
        >{{ border.text.repeat(border.repeat) }}</text
      >
    </view>
    <view
      id="itemLeft"
      :style="{
        height: `${left.height ? left.height : ''}rpx`,
        position: 'absolute',
        overflow: 'hidden',
      }"
    >
      <view
        class="left"
        v-for="n in left.repeat"
        :key="n"
        :style="{ top: `${left.random[0]}rpx` }"
      >
    ...
    <view
      id="itemMain"
      :class="[`${main.row} main-${theme}`]"
      :style="{ height: `${main.height ? main.height : ''}rpx` }"
    >
      <view
        v-if="isJustify && height"
        v-for="(item, index) in main.justify"
        :key="index"
      >
        {{ item }}
      </view>
    ...
    <view class="border">
      <text
        :class="[`text text-${theme}`]"
        :style="{ 'margin-left': `${border.random[1]}rpx` }"
        >{{ border.text.repeat(border.repeat) }}</text
      >
    </view>
  </view>
...
export default {
  ...
  methods: {
    initDom() {
      if (adm.item.updated != adm.screenData.width) {
        ...
        query.exec((res) => {
          ...
          adm.item.unit.main =
            mainHeight / this.$slots.default()[0].children.length;
    ...
    initMain() {
      ...
      const defaultHeight =
        adm.item.unit.main * this.$slots.default()[0].children.length;
      this.main.height = adm.px2rpx(
        this.height ? appointHeight : defaultHeight,
      );
    },
    initLeft() {
...
.item-wrap {
  ...
  .main {
    ...
    &-row {
      ...
      transform-origin: math.div($adm-font-size-base, 2)
        math.div($adm-font-size-base, 2);
